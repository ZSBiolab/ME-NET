...
def InterpretNet_4test(net, x, ylabel,x_test, test_ylabel, Pathway_layer, Hidden_layer,
    ...
    print(len(list(range(Num_Epochs+1))),len(train_losses))
    if not os.path.exists('results4testing'):
        os.makedirs('results4testing')
    if not os.path.exists(os.path.join('results4testing',path_0)):
        os.makedirs(os.path.join('results4testing',path_0))
    torch.save(net.state_dict(), os.path.join('results4testing',path_0,outpath))
    ...
    df.to_csv(os.path.join('results4testing',path_0,'_y_test_y_pred.csv'), index=False)
    df2 = pd.DataFrame({'fpr':fpr,'tpr':tpr,'thresholds':thresholds})
    df2.to_csv(os.path.join('results4testing',path_0,'fpr_tpr.csv'), index=False)
    ...
    df3.to_csv(os.path.join('results4testing',path_0,'other_parameter.csv'), index=False)

    df4 = pd.DataFrame({'Num_Epochs':list(range(Num_Epochs+1)),'train_losses':list(train_losses),'test_losses':list(test_losses)})
    df4.to_csv(os.path.join('results4testing',path_0,'loss-epoch.csv'), index=False)
    ...
    ax.legend()
    plt.savefig(os.path.join('results4testing',path_0,'_roc_curve.pdf')) 
    df0 = pd.DataFrame({'ylabel_real': test_ylabel_np,  'eval_pred': y_pred_np,'pre_scores':y_score_np,"y_pred_rd":y_pred_rd})
    df0.to_csv(os.path.join('results4testing',path_0,'_y_test_y_pred.csv'), index=False)
    df2 = pd.DataFrame({'fpr':fpr,'tpr':tpr,'thresholds':thresholds})
    df2.to_csv(os.path.join('results4testing',path_0,'fpr_tpr.csv'), index=False)
    df3 = pd.DataFrame({'recall':drecall,'precision':dprecision,'f1':df1,'MSE':MSE,'accuracy':daccuracy,'zol':zol,'dhamming_loss':dhamming_loss},index= [1])
    df3.to_csv(os.path.join('results4testing',path_0,'other_parameter.csv'), index=False)
    ...
    plt.style.use('ggplot')
    fig, ax = plt.subplots()
    ax.plot(range(Num_Epochs+1), train_losses, label='Train',color= 'blue')
    ax.plot(range(Num_Epochs+1), test_losses, label='Test',color= 'orange')
    ...
    ax.legend()
    plt.savefig(os.path.join('results4testing',path_0,'_loss_epoch.pdf'))
    plt.close()
